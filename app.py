...
from flask import Flask, render_template, jsonify, request, request, redirect, url_for, session
...
def find_latest_model(prefix):
    """Find the latest cycle model e.g. lstm_cycle2.pkl"""
...
def load_all_models():
    ...
    # Fallback: try lstm_model.pkl if no cycle models found
    ...
    # Set model_info from best available
...
# ── Prediction helpers ─────────────────────────────────────────
def get_confidence(weeks):
    stages = set(w["stage"] for w in weeks)
    if "adult" in stages:
        return "high"
    elif "pupa" in stages and "larvae" in stages:
        return "medium-high"
    elif "pupa" in stages or len(weeks) >= 2:
        return "medium"
    return "low"
...
def predict_single(bundle, col_values):
    ...
    for i, col in enumerate(feature_cols):
        row.append(col_values.get(col, feat_means[i]))
    ...
    try:
        ...
        # LSTM — needs sequence
        seq     = np.repeat(row_sc, seq_days, axis=0).reshape(1, seq_days, len(feature_cols))
        model   = tf.keras.models.model_from_config(bundle["model_config"])
        model.set_weights(bundle["model_weights"])
        ...
        return round(float(base), 1), "cached"


def predict_all_models(col_values):
    ...
    for name, bundle in model_bundles.items():
        ...
        r2  = bundle.get("info", {}).get("r2", 0) if bundle else 0
        results[name] = {
            "predicted_g": pred_g,
            "method":      method,
            "r2":          r2,
            "mae_g":       bundle.get("info", {}).get("mae_g")  if bundle else None,
            "rmse_g":      bundle.get("info", {}).get("rmse_g") if bundle else None,
            "mape":        bundle.get("info", {}).get("mape")   if bundle else None,
        }

    # Fallback formula if all None
    ...
    best_name = max(available, key=lambda k: available[k]["r2"] or 0)
    results["best_model"] = best_name
    return results


def build_col_values_from_weeks(weeks):
    """Aggregate weekly data into col_values dict for prediction."""
    stage_data = {}
    for w in weeks:
        s = w["stage"]
...
def predict_from_weeks(weeks):
    """Predict from all weeks — returns all_preds dict."""
    col_values = build_col_values_from_weeks(weeks)
    return predict_all_models(col_values)
...
def api_cycle_week_post():
    ...
    # Save prediction on the week entry
...
# ── NEW: Cycle restart route ───────────────────────────────────
@app.route("/api/cycle/restart", methods=["POST"])
def api_cycle_restart():
    """Reset everything back to Cycle 1, clearing all weekly data."""
    global cycle_data
